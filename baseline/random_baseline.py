...
_, root_save_path = prepare_saving(args.saving_folder, 'random_baseline', None, None)
...
def predict(output_shape, num_labels):
    predictions = torch.randint(num_labels, size=output_shape)
    return predictions
...
def _main():
    ...
    predictions = predict(test_labels.shape, len(full_labels))
